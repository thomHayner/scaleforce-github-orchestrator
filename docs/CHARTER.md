...
1. **Review loop orchestration** ([ADR 0005](adr/0005-copilot-reviewer-loop.md)). Request reviews from Copilot, poll for review arrival, triage threads into the six-outcome taxonomy (triage-to-bucket classification is the allowed narrow-LLM case), dispatch FIX work to a code-authoring handler (which decides, edits, verifies, and pushes the commit under its own identity per [ADR 0003](adr/0003-bot-identity-separation.md#actor-provenance-invariant)), open Discussions for DISCUSS and Issues for DEFER, reply on and resolve threads, re-request review on new HEAD, detect non-convergence. scaleforce owns the orchestration writes (review request, replies, resolutions, Discussion / Issue creation); the FIX commit itself is authored and pushed by the handler, not by scaleforce.
...
- **Does not author code.** Every authoring write is attributable to a code-authoring handler (claude[bot], copilot-swe-agent[bot], future codex). scaleforce never pushes to protected or default branches. It pushes commits to PR branches in only two bounded cases: (a) opening or updating a scaffolding / templating PR whose contents are pure scaffold or template output, and (b) acting as the GitHub transport for a FIX commit produced by a code-authoring handler in the review loop — in which case the handler is the code author and scaleforce is the orchestrator, never the reverse. Anything beyond these two cases is a scope violation.
